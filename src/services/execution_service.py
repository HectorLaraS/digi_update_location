...
class ExecutionService:
    ...
    def execute(
        ...
        ready_routers = [
            r for r in routers if r.system_status == "ready"
        ]
        ...
        for index, router in enumerate(ready_routers, start=1):
            ...
            # Delay entre routers (si habrá reboot)
    ...
    def _process_single_router(
        ...
        reboot_result = self._digi.reboot_device(router.device_id)
        ...
        success = poll_until(
            lambda: self._is_router_connected(router.device_id),
    ...
    def _is_router_connected(self, device_id: str) -> bool:
        status = self._digi.get_connection_status_by_id(device_id)
